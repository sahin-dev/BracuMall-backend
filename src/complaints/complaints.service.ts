...
export class ComplaintsService {
  ...
  private async resolveAgainstUserId(filedById: string, input: CreateComplaintInput): Promise<string> {
    if (input.targetType === 'order') {
      if (!input.targetId) throw new BadRequestException('targetId is required for an order complaint');
      const order = await this.prisma.order.findUniqueOrThrow({ where: { id: input.targetId } });
    ...
    if (input.targetType === 'product') {
      if (!input.targetId) throw new BadRequestException('targetId is required for a product complaint');
      const product = await this.prisma.product.findUniqueOrThrow({ where: { id: input.targetId } });
      return product.sellerId;
    }
    if (input.targetType === 'store') {
      if (!input.targetId) throw new BadRequestException('targetId is required for a store complaint');
      const store = await this.prisma.store.findUniqueOrThrow({ where: { id: input.targetId } });
      return store.ownerId;
    }
    // user
    if (!input.targetId) throw new BadRequestException('targetId is required for a user complaint');
  ...
  async create(input: CreateComplaintInput, filedById: string) {
    ...
    if (againstUserId === filedById) throw new BadRequestException('Cannot file a complaint against yourself');
    ...
    const complaint = await this.prisma.complaint.create({
      data: {
        ...
        occurrenceAt: input.occurrenceAt ? new Date(input.occurrenceAt) : undefined,
  ...
  async updateStatus(id: string, dto: { status: string; resolution?: string }, adminId: string) {
    const complaint = await this.prisma.complaint.findUniqueOrThrow({ where: { id } });
    ...
    await this.notifications.createForMany([complaint.filedById, complaint.againstUserId], {
  ...
  async respond(id: string, userId: string, response: string) {
    const complaint = await this.prisma.complaint.findUniqueOrThrow({ where: { id } });
    if (complaint.againstUserId !== userId)
